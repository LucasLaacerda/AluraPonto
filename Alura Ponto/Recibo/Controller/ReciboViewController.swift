...
class ReciboViewController: UIViewController {
    ...
    private lazy var controladorDeIamgem = UIImagePickerController()
    ...
    func mostraMenuEscolhaDeFoto() {
        let menu = UIAlertController(title: "Seleçao de foto", message: "Escolha uma foto da biblioteca", preferredStyle: .actionSheet)
        menu.addAction(UIAlertAction(title: "Biblioteca de fotos", style: .default, handler: {
            action in
            if UIImagePickerController.isSourceTypeAvailable(.photoLibrary){
                self.camera.delegate = self
                self.camera.abrirBibliotecaFotos(self, self.controladorDeIamgem)
            }
        }))
...
extension ReciboViewController: CameraDelegate {
    func didSelectFoto(_ image: UIImage) {
        escolhaFotoButton.setImage(image, for: .normal)
